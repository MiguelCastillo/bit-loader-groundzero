...
define([
  ...
  var _result = Utils.result;
  var _noop   = Utils.noop;
  ...
  */
  Module.define = function() {
    var m = Module.adapters.apply({anonymous:[], modules: {}}, arguments);

    // Only named modules can be registered directly
    if ( m.name && !pending[m.name] ) {
      pending[m.name] = {name: m.name, deps: m.deps, factory: m.factory};
    }
  };
  ...
  */
  Module.load = function(moduleMeta) {
    return Module.fetch(moduleMeta).then(function(moduleContent) {
  ...
  */
  ...
  Module.configure = function (settings) {
    Module.settings.paths = settings.paths;
    return Module;
  };
